...
The energy balance should close according to:
...
.. code-block:: python
   ...
   total_energy = SW_net + LW_net + Q_sensible + Q_latent + Gf + Q_precip

   # Should approximately equal Energy variable
   energy_difference = total_energy - Energy
...
.. code-block:: python
   ...
   # Mass balance check
   mass_input = SnowfallWaterEq + RaininSnow + Condensation
   mass_output = SnowMelt + Sublimation + Runoff
   net_change = mass_input - mass_output + RefrozenWater

   # Should equal change in SWE + PackWater
